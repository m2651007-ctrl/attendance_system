...
# cache لمنع تكرار Register محاولات الحضور المكررة
import time
...
def register_kiosk_routes(app):
    ...
    def kiosk_authorize():
        ...
        if not room_name or not room_code:
            return jsonify({"ok": False, "message": "Room والكود مطلوبان"})
        ...
        if not row:
            return jsonify({"ok": False, "message": "Room أو الكود غير صحيح"})
        ...
        try:
            ...
            cur.execute("""
                ...
                WHERE s.room_name = ?
                  ...
                  AND s.active    = 1
                  AND (
                    CAST(SUBSTR(s.start_time,1,INSTR(s.start_time,':')-1) AS INTEGER)*60 +
                    CAST(SUBSTR(s.start_time,INSTR(s.start_time,':')+1)   AS INTEGER)
                  ) - 60 <= ?
                  AND (
                    CAST(SUBSTR(s.end_time,1,INSTR(s.end_time,':')-1) AS INTEGER)*60 +
                    CAST(SUBSTR(s.end_time,INSTR(s.end_time,':')+1)   AS INTEGER)
                  ) + 15 >= ?
                LIMIT 1
            """, (room_name, room_code, now_min, now_min))
            row = cur.fetchone()
        finally:
            if conn: conn.close()

        if not row:
            return jsonify({"ok": False, "message": "No active lecture Active الآن"})
    ...
    def kiosk_capture():
        ...
        try:
            ...
            cur.execute("""
                ...
                WHERE room_name = ? AND room_code = ?
                  AND active = 1
                  AND (
                    CAST(SUBSTR(start_time,1,INSTR(start_time,':')-1) AS INTEGER)*60 +
                    CAST(SUBSTR(start_time,INSTR(start_time,':')+1)   AS INTEGER)
                  ) - 60 <= ?
                  AND (
                    CAST(SUBSTR(end_time,1,INSTR(end_time,':')-1) AS INTEGER)*60 +
        ...
        if not session_row:
            return jsonify({"ok": False, "message": "No active lecture Active"})
        ...
        if not spoof_result.is_live:
            try:
                log_admin_activity(
                    "UNKNOWN", "Unknown", "SPOOF_ATTEMPT_DETECTED",
            ...
            return jsonify({
                ...
                "message": "تم Reject الوجه — لا يبدو وجهاً حقيقياً"
            })
        ...
        try:
            ...
            if mode == "checkout":
                ...
                if att["check_out"]:
                    return jsonify({
                        ...
                        "box": [int(x), int(y), int(w), int(h)],
                        "checked_out": False,
                        "message": "تم Record Checkout مسبقًا"
                    })
                ...
                if cache_key not in _duplicate_cache or (now_ts - _duplicate_cache[cache_key]) > _DUPLICATE_COOLDOWN:
                    ...
                    try:
                        log_admin_activity(uid, student_info["name"], "DUPLICATE_ATTENDANCE_ATTEMPT",
                            session_id=active_session_id, details="محاولة Duplicate Attendanceة")
                    except Exception:
                        pass
                return jsonify({
                    "ok": True, "recognized": True,
            ...
            # تحديد حالة الحضور (Present أو Late)
            try:
