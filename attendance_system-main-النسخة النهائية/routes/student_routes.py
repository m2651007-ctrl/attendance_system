...
def _student_status_is_absent(status_value):
    ...
    if not raw:
        return False
    return any(token in raw for token in ["absent", "غياب", "Absent"])
...
def _student_validate_strong_password(password_value):
    ...
    if len(password_value) < 8:
        return False, "Password يجب أن تكون 8 أحرف على الأقل"
    if not any(ch.islower() for ch in password_value):
        return False, "Password يجب أن تحتوي على حرف صغير واحد على الأقل"
    if not any(ch.isupper() for ch in password_value):
        return False, "Password يجب أن تحتوي على حرف كبير واحد على الأقل"
    if not any(ch.isdigit() for ch in password_value):
        return False, "Password يجب أن تحتوي على رقم واحد على الأقل"
    if not any(not ch.isalnum() for ch in password_value):
        return False, "Password يجب أن تحتوي على رمز خاص واحد على الأقل"
    return True, ""
...
def _student_collect_page_data(student_id, msg="", msg_type="info", process_alerts=False):
    ...
    alerts = []
    if total_sessions == 0:
        alerts.append({"type": "info", "text": "No records found حضور حتى الآن."})
    ...
    if risk_level in ("warning", "deprived") and not student_email:
        alerts.append({"type": "info", "text": "لا يوجد بريد إلكتروني مسجل لتفعيل تنبيهات Email."})
...
def register_student_routes(app):
    ...
    def student_login():
        if request.method == "POST":
            ...
            if not uid or not password:
                return render_template("student/student_login.html", error="University ID وPassword مطلوبان")
            ...
            if not stu:
                return render_template("student/student_login.html", error="University ID أو Password غير صحيحة")
    ...
    def student_update_email():
        ...
        if email_value and not _student_is_valid_email(email_value):
            return redirect("/student/alerts?msg=Email Address غير صالح&msg_type=danger")
        ...
        return redirect("/student/alerts?msg=تم Refresh Email Address&msg_type=success")
    ...
    def student_update_password():
        ...
        if not current_password or not new_password or not confirm_password:
            return redirect("/student/alerts?msg=جميع حقول Password مطلوبة&msg_type=danger")
        if new_password != confirm_password:
            return redirect("/student/alerts?msg=Confirm Password غير مطابق&msg_type=danger")
        if current_password == new_password:
            return redirect("/student/alerts?msg=New Password يجب أن تكون مختلفة عن الحالية&msg_type=warning")
        ...
        try:
            ...
            if student_row["password_hash"] != _student_hash_password(current_password):
                return redirect("/student/alerts?msg=Current Password غير صحيحة&msg_type=danger")
        ...
        return redirect("/student/alerts?msg=تم Change Password بنجاح&msg_type=success")
    ...
    def student_create_appeal():
        ...
        if len(reason) < 10:
            return redirect("/student/attendance?msg=Appeal Reason يجب أن يكون أوضح (10 أحرف على الأقل)&msg_type=danger")
        if len(reason) > 1000:
            return redirect("/student/attendance?msg=Appeal Reason طويل جدًا&msg_type=danger")
        ...
        try:
            ...
            if not attendance_row:
                return redirect("/student/attendance?msg=Attendance Record غير موجود&msg_type=danger")
