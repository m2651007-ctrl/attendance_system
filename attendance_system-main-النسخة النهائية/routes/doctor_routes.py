...
def register_doctor_routes(app):
    ...
    def doctor_login():
        if request.method == "POST":
            ...
            return render_template("doctor/login.html", error="Invalid username or password")
    ...
    def export_attendance():
        ...
        session_id = request.args.get("session_id")
        if not session_id:
            return "يرجى تحديد المPresentة", 400
        ...
        writer.writerow(["Name","الرقم الأكاديمي","University ID","Check-in Time","Check-out Time","Status"])
        for r in rows:
            writer.writerow([
    ...
    def report_status():
        ...
        details = f"Status Type: {status_type}"
        if note:
            details += f" | ملاحظة: {note}"
        details += f" | المادة: {s_row['course_name']} | Instructor: {session['doctor_name']}"
