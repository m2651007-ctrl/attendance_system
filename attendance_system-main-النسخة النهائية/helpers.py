...
# ================= CONFIG =================
# Detection scale: 0.7 preserves more detail from low-res mobile frames
# (was 0.5 — too aggressive downscaling caused missed detections on Android)
FACE_DETECT_SCALE = 0.7

# Minimum face pixel size for detection.
# Reduced from (80,80) to (40,40) so small/distant faces on mobile cameras
# are still accepted. The embedding step will resize to 160×160 anyway.
FACE_MIN_SIZE = (40, 40)

# Recognition distance threshold. Slightly relaxed from 0.9 to 1.0
# to accommodate natural variation from different devices/lighting.
# Still rejects strangers — just less likely to reject the same person
# due to compression artifacts or phone camera noise.
RECOG_THRESHOLD = 1.0

# Total target photo count for registration.
# Kept at 12 so the embedding average is rich and diverse.
TARGET_REG_IMAGES = 12

# Minimum VALID photos required before we accept the registration.
# Changed from requiring ALL 12 to requiring at least 6.
# This means the system will succeed even if some frames are blurry/rejected,
# as long as at least 6 good embeddings are extracted.
MIN_REG_IMAGES = 6
...
def log_admin_activity(student_id, student_name, action, session_id=None, details=None):
    ...
    try:
        ...
        print(f"Error في Register نشاط الإدارة: {e}")
    finally:
        if conn:
            conn.close()